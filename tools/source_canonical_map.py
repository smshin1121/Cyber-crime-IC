...
def render_report(rows: list[SourceRecord], groups: list[CanonicalGroup]) -> str:
    ...
    lines = [
        ...
        "## Summary",
        "",
        "This map selects a canonical candidate for each duplicated `collection_url`. It is intentionally advisory: duplicate source pages are not deleted here, and pages should be deduplicated only when the same URL is repeated inside one operation/case reference list.",
        "",
        ...
        "- Keep all alternates addressable until a human review decides whether to mark them `duplicate_of` or `absorbed_into`.",
        "",
