...
def source_collection_url(slug: str) -> str:
    path = SOURCES_DIR / f"{slug}.md"
    if not path.exists():
        return ""
    try:
...
def audit_page(path: Path, category: str) -> DepthRow:
    ...
    source_urls = [source_collection_url(slug) for slug in sources]
    duplicate_source_urls = sum(count - 1 for count in Counter(url for url in source_urls if url).values() if count > 1)
    status = str(meta.get("status") or "").strip().lower()
    operation_role = str(meta.get("operation_role") or "").strip().lower()
    has_canonical_pointer = bool(
        re.search(r"^## Canonical Record\b", body_without_refs, re.I | re.M)
        or meta.get("parent_operation")
        or as_list(meta.get("related_cases"))
        or meta.get("related_operation")
    )
    is_absorbed_wrapper = status == "absorbed" and has_canonical_pointer
    ...
    if is_absorbed_wrapper:
        if source_count > 1:
            issues.append(f"absorbed_wrapper_overreferenced:{source_count}")
            score += min(20, 8 + source_count * 2)
        if body_words < 35:
            issues.append(f"absorbed_wrapper_too_sparse:{body_words}w")
            score += 8
        if operation_role and operation_role != "follow-on":
            issues.append(f"absorbed_wrapper_role_check:{operation_role}")
            score += 5
    elif placeholder_hits:
        issues.append(f"placeholder_language:{placeholder_hits}")
        score += min(30, 10 + placeholder_hits * 3)

    if not is_absorbed_wrapper and source_count >= 5 and body_words < 350:
        issues.append(f"source_rich_thin_body:{body_words}w")
        score += 35
    elif not is_absorbed_wrapper and source_count >= 3 and body_words < 220:
        issues.append(f"thin_body:{body_words}w")
        score += 22

    if not is_absorbed_wrapper and source_count >= 5 and quality_sections < 4:
        issues.append(f"missing_quality_sections:{quality_sections}/6")
        score += 18

    if not is_absorbed_wrapper and source_words >= 1200 and body_words < 450:
        issues.append(f"raw_available_underused:{source_words}w_sources")
        score += 22

    if not is_absorbed_wrapper and source_count >= 5 and source_words >= 2500 and body_words < 700:
        issues.append("high_source_to_body_gap")
        score += 12

    if duplicate_source_urls:
        issues.append(f"duplicate_source_url_refs:{duplicate_source_urls}")
        score += min(20, 8 + duplicate_source_urls * 2)

    if category == "operations" and str(meta.get("title_ko") or "").startswith("공식 작전명 없음"):
        issues.append("unresolved_operation_name")
        score += 8

    if not is_absorbed_wrapper and "Enforcement Action" in title and body_words < 500:
        issues.append("generated_enforcement_title")
        score += 10

    crimes = crime_values(meta)
    if not is_absorbed_wrapper:
...
def render_report(rows: list[DepthRow]) -> str:
    ...
    placeholders = [row for row in rows if any(i.startswith("placeholder_language") for i in row.issues)]
    mismatch = [row for row in rows if any(i.startswith("possible_crime_type_mismatch") for i in row.issues)]
    duplicate_refs = [row for row in rows if any(i.startswith("duplicate_source_url_refs") for i in row.issues)]
    absorbed_overreferenced = [row for row in rows if any(i.startswith("absorbed_wrapper_overreferenced") for i in row.issues)]
    ...
    lines = [
        ...
        f"- Pages with duplicate source URLs in references: **{len(duplicate_refs)}**",
        f"- Absorbed wrapper pages with more than one source: **{len(absorbed_overreferenced)}**",
        "",
        "## Issue Counts",
        "",
        "| Issue | Count |",
    ...
    for row in duplicate_refs[:80]:
        issue = next(i for i in row.issues if i.startswith("duplicate_source_url_refs"))
        count = issue.split(":", 1)[1] if ":" in issue else ""
        lines.append(f"| [[{row.slug}]] | {row.category[:-1]} | {row.source_count} | {count} |")
    ...
    for row in absorbed_overreferenced[:80]:
        issue = next(i for i in row.issues if i.startswith("absorbed_wrapper_overreferenced"))
        lines.append(f"| [[{row.slug}]] | {row.category[:-1]} | {row.source_count} | {row.body_words} | {issue} |")
