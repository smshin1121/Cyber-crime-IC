---
title: "Europol: Coordinated action cuts off access to VPN service used by ransomware groups"
collection_source: Europol
collection_url: https://www.europol.europa.eu/media-press/newsroom/news/coordinated-action-cuts-access-to-vpn-service-used-ransomware-groups
final_url: https://www.europol.europa.eu/media-press/newsroom/news/coordinated-action-cuts-access-to-vpn-service-used-ransomware-groups
collection_domain: europol.europa.eu
collection_date: 2026-04-27
publish_date: 2021-06-30
language: en
status: materialized
text_status: summarized
storage_mode: summary-only
source_type: press-release
source_page: wiki/sources/europol-doublevpn-takedown.md
word_count: 322
content_hash: sha256:b623bd8f68c65ca68214b8e14486329e247efd3be8c2ba8f02f256248825785e
extraction_date: 2026-04-27
copyright_policy: summary-only
---
## Source Archive Record

This record preserves the source metadata and source-page digest so the source corpus has a separate addressable record for this reference.

## Source Digest

- Title: Europol: Coordinated action cuts off access to VPN service used by ransomware groups
- Publisher: Europol
- Source type: press-release
- Publication date: 2021-06-30
- Linked pages: doublevpn-takedown, europol-ec3, eurojust, netherlands

### Key Findings
- DoubleVPN web domains and server infrastructure seized across 9 countries on 29 June 2021
- Dutch National Police led the operation under the Dutch National Public Prosecutor's Office; Europol EC3 organized 30+ coordination meetings and 4 workshops
- Eurojust organized 6 dedicated coordination meetings since October 2020 and established a coordination centre on the action day
- DoubleVPN offered single/double/triple/quadruple VPN connections from €22, marketed on Russian and English cybercrime forums
- Customer personal information, logs, and statistics were seized — enabling follow-on investigations
- No arrests announced; first law enforcement takedown of a criminal-enabling VPN service
- Participating agencies: Dutch Police, FBI, US Secret Service, DOJ, RCMP, BKA, NCA, Swedish Police, Italian State Police, Bulgarian GDOC, Cantonal Police Ticino
- Operating under EMPACT framework; Dutch 'hackbevoegdheid' (digital intrusion power) used

### Source Page Summary

- **Publisher**: Europol (European Law Enforcement Agency) - **URL**: <https://www.europol.europa.eu/media-press/newsroom/news/coordinated-action-cuts-access-to-vpn-service-used-ransomware-groups> - **Published**: 2021-06-30 - **Source Tier**: 1 (official coordinating agency press release) - **Reliability**: high - **Credibility**: confirmed (corroborated by Eurojust and NCA press releases same day) - **Operation**: DoubleVPN Takedown Europol's press release announces the coordinated seizure of DoubleVPN infrastructure on 29 June 2021 across 9 countries, led by the Dutch National Police. Key details: 1. **30+ coordination meetings** and **4 workshops** organized by EC3 2. **6 Eurojust judicial coordination meetings** since October 2020 3. **Virtual command post** on the action day 4. Service marketed from **€22** on cybercrime forums 5. Customer logs and data seized for follow-on.

## Extraction Notes

- storage_mode: summary-only
- source_page: wiki/sources/europol-doublevpn-takedown.md
- generated_at: 2026-04-27T13:52:26+00:00
- full_text: not fetched in this materialization pass
- rights_mode: summary-only public corpus record
