---
type: source
title: "Administrator of Darkode Hacking Forum Sentenced to Prison"
source_type: news
publisher: SecurityWeek
author: "Eduard Kovacs"
publish_date: 2023-11-16
ingest_date: 2026-04-25
language: en
reliability: medium
credibility: reported
sensitivity: public
pages_updated:
  - operation-us-v-notorious-hacker
key_findings:
  - "Identifies defendant as Thomas Kennedy McCormick (Darkode 'Fubar')."
  - "30,000 stolen credit cards seized at residence."
collection_url: https://www.securityweek.com/administrator-of-darkode-hacking-forum-sentenced-to-prison/
created: 2026-04-25
raw_path: raw/news/2023-11-16_securityweek_administrator-darkode-hacking-forum-sentenced-to-prison.md
copyright_policy: summary-only
---
## Source Summary

SecurityWeek cyber trade press de-anonymizing the DOJ "notorious hacker" press release as Darkode admin Thomas McCormick.

## Relevance to IC

Cyber trade press for [[operation-us-v-notorious-hacker]] — links to international Operation Shrouded Horizon (Darkode takedown).
