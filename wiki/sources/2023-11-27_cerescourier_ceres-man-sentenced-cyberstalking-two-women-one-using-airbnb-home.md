---
type: source
title: "Ceres man sentenced for cyberstalking two women, one using Airbnb home"
source_type: news
publisher: "Ceres Courier"
author: "Staff"
publish_date: 2023-11-27
ingest_date: 2026-04-25
language: en
reliability: medium
credibility: probably-true
sensitivity: public
pages_updated:
  - operation-us-v-kevin-james-strutz
key_findings:
  - "Hometown coverage adds FBI Nov 2020 search-warrant attic-barricade detail and prior record."
collection_url: https://www.cerescourier.com/news/local/ceres-man-sentenced-cyberstalking-two-women-one-using-airbnb-home/
created: 2026-04-25
---
## Source Summary

Ceres Courier local coverage of the Strutz federal sentencing, adding pre-sentencing investigation detail (FBI search warrant, defendant's prior criminal record).

## Relevance to IC

Independent local hometown press coverage for [[operation-us-v-kevin-james-strutz]]. Domestic; no IC element.
