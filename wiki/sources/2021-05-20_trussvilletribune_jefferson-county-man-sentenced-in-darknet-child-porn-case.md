---
type: source
title: "Jefferson County man sentenced in Darknet child porn case"
source_type: news
publisher: "The Trussville Tribune"
author: Staff
publish_date: 2021-05-20
ingest_date: 2026-04-25
language: en
reliability: low
credibility: reported
sensitivity: public
pages_updated:
  - operation-jefferson-county-man-sentenced-to-6-12-years-in-prison-for-possession-of-child-pornography
  - operation-us-v-james-curtis-brasher
key_findings:
  - "Headline framing as 'Darknet child porn case' adds explicit Tor/Darknet attribution"
collection_url: https://www.trussvilletribune.com/2021/05/20/jefferson-county-man-sentenced-in-darknet-child-porn-case/
created: 2026-04-25
raw_path: raw/news/2021-05-20_trussvilletribune_jefferson-county-man-sentenced-in-darknet-child-porn-case.md
copyright_policy: summary-only
---
## Source Summary

Hyperlocal Birmingham-area weekly with explicit Darknet framing in headline. Direct fetch returned HTTP 500; content confirmed via search engine snippets and corroborating local coverage.

## Relevance to IC

Hyperlocal press coverage demonstrating that the Darknet aspect was deemed newsworthy at the local-paper level.
