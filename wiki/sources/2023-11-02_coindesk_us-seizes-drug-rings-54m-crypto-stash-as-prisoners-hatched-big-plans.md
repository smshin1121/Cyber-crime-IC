---
type: source
title: "Prisoners' Dreams to Stash $54M in Crypto in Exotic Locales Dashed as Feds Grab It for Treasury"
source_type: news
publisher: CoinDesk
author: "Jesse Hamilton"
publish_date: 2023-11-02
ingest_date: 2026-04-25
language: en
reliability: high
credibility: reported
sensitivity: public
pages_updated:
  - operation-united-states-seized-and-files-forfeiture-action-to-recover-over-54-million-of-cryptocurrency-traceable-to-nar
key_findings:
  - "Documents prisoner-call recordings about plans to relocate ETH to Malta, Ireland, Bahamas"
  - "FBI Newark Special Agent quoted on tracing illicit cryptocurrency"
collection_url: https://www.coindesk.com/policy/2023/11/02/us-seizes-drug-rings-54m-crypto-stash-as-prisoners-hatched-big-plans-for-assets
created: 2026-04-25
raw_path: raw/news/2023-11-02_coindesk_us-seizes-drug-rings-54m-crypto-stash-as-prisoners-hatched-big-plans.md
text_status: summarized
content_hash: sha256:f8434c00336303feee4ca1cf8800581201d2b63d10bf1f051979b17b851ca63f
word_count: 748
stored_word_count: 80
extraction_date: 2026-04-25
last_fetcher: urllib
copyright_policy: summary-only
---
## Source Summary

CoinDesk's policy desk covers the District of New Jersey civil forfeiture, with detail on Castelluzzo's prison-call planning to relocate the ETH holdings overseas — material relevant to IC because it shows the offshore-evasion intent that the U.S. forfeiture preempted.

## Relevance to IC

Major crypto-policy outlet confirmation of the asset-recovery action with detail on attempted offshore evasion (Malta, Ireland, Bahamas) — directly relevant to IC analysis for [[operation-united-states-seized-and-files-forfeiture-action-to-recover-over-54-million-of-cryptocurrency-traceable-to-nar]].
