---
type: source
title: "Dark Web Fentanyl Distributor Sentenced To More Than 15 Years In Federal Prison"
source_type: press-release
publisher: "PublicNow (USAO MDFL syndication)"
author: "USAO Middle District of Florida"
publish_date: 2025-04-10
ingest_date: 2026-04-25
language: en
reliability: high
credibility: confirmed
sensitivity: public
pages_updated:
  - operation-us-v-bookman-dark-web
key_findings:
  - "Syndicated USAO MDFL press release content."
  - "Provides bot-wall-resilient retrievability of the official press release."
collection_url: https://www.publicnow.com/view/FECC9EEC5790CAAF29D8C73DBD6EE07F0C457B8D
created: 2026-04-25
raw_path: raw/press-releases/2025-04-10_publicnow_dark-web-fentanyl-distributor-sentenced-more-than-15-years.md
copyright_policy: summary-only
---
## Source Summary

PublicNow syndicated republication of the USAO MDFL press release on Bookman's sentencing.

## Relevance to IC

This source provides syndicated official-press-release retrievability for [[operation-us-v-bookman-dark-web]] when the justice.gov original is bot-walled.
