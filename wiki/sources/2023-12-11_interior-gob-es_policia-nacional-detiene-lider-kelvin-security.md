---
type: source
title: "La Policía Nacional detiene al líder del aparato financiero de uno de los grupos hacktivistas más importantes del mundo"
collection_url: https://www.interior.gob.es/opencms/en/detail-pages/article/La-Policia-Nacional-detiene-al-lider-del-aparato-financiero-de-uno-de-los-grupos-hacktivistas-mas-importantes-del-mundo/
collection_domain: interior.gob.es
source_type: press-release
publisher: "Ministry of the Interior of Spain"
author: ""
publish_date: 2023-12-11
ingest_date: 2026-04-18
language: en
reliability: high
credibility: confirmed
sensitivity: public
source_tier: 1
pages_updated:
  - blackcat-lockeroga-kelvin-security-2023
key_findings:
  - "Spain's National Police announced the arrest in Alicante of a person allegedly linked to Kelvin Security."
  - "The release described Kelvin Security as responsible for attacks on more than 300 organizations in more than 90 countries over three years."
  - "The public statement identified alleged offenses including criminal organization membership, disclosure of secrets, computer damage, and money laundering."
created: 2026-04-18
updated: 2026-04-18
---
## Source Summary

This is the official Spanish government source for the December 2023 arrest linked to Kelvin Security.

## Relevance to IC

This source supports the Kelvin Security portion of the composite December 2023 enforcement page and replaces the prior `N/A` placeholder.
