---
type: source
title: "FEDS STOP METH DRUG RING ON DARK WEB, SEIZE $5 MILLION IN ASSETS DURING ARREST OF TWO MEN"
source_type: news
publisher: "Front Page Detectives"
author: Staff
publish_date: 2021-02-23
ingest_date: 2026-04-25
language: en
reliability: low
credibility: reported
sensitivity: public
pages_updated:
  - operation-two-men-charged-in-multi-million-dollar-darknet-drug-distribution-conspiracy
  - operation-us-v-kevin-ombisi
  - operation-us-v-kevin-olando-ombisi
  - operation-texas-residents-sentenced-for-their-involvement-in-a-counterfeit-prescription-drug-distribution-operation
key_findings:
  - "Documents Ombisi/Russell role split: Ombisi handled shipping, Russell controlled the Bitcoin wallet"
  - "Confirms multi-state controlled purchases"
collection_url: https://www.frontpagedetectives.com/p/feds-meth-adderall-dark-web-charge-tennessee
created: 2026-04-25
raw_path: raw/news/2021-02-23_frontpagedetectives_feds-meth-adderall-dark-web-charge-tennessee.md
copyright_policy: summary-only
---
## Source Summary

Front Page Detectives provides true-crime-style coverage of the WDTN darknet drug indictment with clearer role attribution between the two defendants.

## Relevance to IC

Adds role-separation detail useful for SNA edge characterization between the two defendants.
