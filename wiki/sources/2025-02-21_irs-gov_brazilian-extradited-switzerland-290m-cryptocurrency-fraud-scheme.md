---
type: source
title: "Brazilian extradited from Switzerland to the United States to face indictment charging involvement in $290m+ cryptocurrency fraud scheme"
source_type: press-release
publisher: "IRS Criminal Investigation"
author: "Staff"
publish_date: 2025-02-21
ingest_date: 2026-04-25
language: en
reliability: high
credibility: confirmed
sensitivity: public
pages_updated:
  - operation-brazilian-extradited-from-switzerland-to-the-united-states-to-face-indictment-charging-involvement-in-290m-cry
key_findings:
  - "IRS-CI parallel press release confirming Douver T. Braga's extradition from Switzerland to Seattle on a 13-count indictment for wire fraud and conspiracy."
  - "Identifies IRS-CI as a lead investigative agency alongside FBI in the Trade Coin Club (TCC) Ponzi investigation."
collection_url: https://www.irs.gov/compliance/criminal-investigation/brazilian-extradited-from-switzerland-to-the-united-states-to-face-indictment-charging-involvement-in-290m-plus-cryptocurrency-fraud-scheme
created: 2026-04-25
duplicate_of: "[[2025-02-21_irs-gov-ci_brazilian-extradited-from-switzerland-to-the-united-states-290m-cryptocurrency-fraud]]"
duplicate_reason: same_collection_url
duplicate_key: "https://www.irs.gov/compliance/criminal-investigation/brazilian-extradited-from-switzerland-to-the-united-states-to-face-indictment-charging-involvement-in-290m-plus-cryptocurrency-fraud-scheme"
duplicate_normalized_at: 2026-04-26
---
## Source Summary

IRS Criminal Investigation issued a parallel press release on the same date as the USAO-WDWA announcement, confirming Braga's extradition from Switzerland and the IRS-CI investigative role.

## Relevance to IC

This source provides parallel agency confirmation for [[operation-brazilian-extradited-from-switzerland-to-the-united-states-to-face-indictment-charging-involvement-in-290m-cry]].
