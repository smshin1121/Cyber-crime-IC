---
type: source
title: "Three men charged in connection to alleged Ponzi scheme tied to Derby business"
source_type: news
publisher: KAKE
author: Staff
publish_date: 2025-12-17
ingest_date: 2026-04-25
language: en
reliability: medium
credibility: reported
sensitivity: public
pages_updated:
  - operation-men-indicted-for-crimes-related-to-securities-fraud
  - operation-us-v-steve-parish-richard-dean-and-joshua-owen
key_findings:
  - "Adds context: ~$100M raised from 570+ investors across 15 states; funds allegedly diverted to Belize/Las Vegas real estate, aircraft"
  - "Direct fetch rate-limited (HTTP 429); details verified via WebSearch snippet"
collection_url: https://www.kake.com/home/three-men-charged-in-connection-to-alleged-ponzi-scheme-tied-to-derby-business/article_198cb213-149d-43f4-a36f-97f35e4995dd.html
created: 2026-04-25
raw_path: raw/news/2025-12-17_kake_three-men-charged-ponzi-scheme-derby-business.md
copyright_policy: summary-only
---
## Source Summary

KAKE coverage providing the largest scope figures (570 investors, $100M, 15 states, Belize) absent from other reports.

## Relevance to IC

Independent press coverage adding cross-border (Belize) angle for [[operation-men-indicted-for-crimes-related-to-securities-fraud]] and [[operation-us-v-steve-parish-richard-dean-and-joshua-owen]].
