---
type: source
title: "Large-scale fraud using trusted online seller accounts uncovered"
raw_path: raw/press-releases/2025-06-25_eurojust-europa-eu_large-scale-fraud-using-trusted-online-seller-accounts-uncovered.md
source_type: press-release
publisher: Eurojust
author: ""
publish_date: 2025-06-25
ingest_date: 2026-04-17
language: en
reliability: high
credibility: confirmed
sensitivity: public
pages_updated:
  - operation-germany-romania-trusted-seller-fraud-2025
key_findings:
  - "German and Romanian authorities dismantled a cyber-enabled fraud group that hijacked trusted online seller accounts and used them to sell goods that would never arrive. Eurojust supported the operation through a **joint investigation team** and later through **European Arrest Warrant** procedures."
collection_url: https://www.eurojust.europa.eu/news/large-scale-fraud-using-trusted-online-seller-accounts-uncovered
created: 2026-04-17
text_status: source-digest
storage_mode: source-digest
content_hash: sha256:acabfb6ec2c573ff2bb2af9ea290069580d5a3d3a67bfe29a3c50118f35cee1b
word_count: 171
extraction_date: 2026-04-28
harvest_status: fulltext_not_available
harvest_note: "Repeated live fulltext harvesting failed or is not suitable; source digest retained."
last_fetch_attempt: 2026-04-29
---
## Source Summary

German and Romanian authorities dismantled a cyber-enabled fraud group that hijacked trusted online seller accounts and used them to sell goods that would never arrive. Eurojust supported the operation through a **joint investigation team** and later through **European Arrest Warrant** procedures.

## Relevance to IC

The source supports [[operation-germany-romania-trusted-seller-fraud-2025]] in the `operations` corpus.