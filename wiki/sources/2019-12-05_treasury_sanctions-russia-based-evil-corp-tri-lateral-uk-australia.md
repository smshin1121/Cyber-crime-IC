---
type: source
title: "Treasury Sanctions Members of the Russia-Based Cybercriminal Group Evil Corp in Tri-Lateral Action with the United Kingdom and Australia"
source_type: press-release
publisher: "U.S. Department of the Treasury"
author: "Treasury Press Office"
publish_date: 2019-12-05
ingest_date: 2026-04-25
language: en
reliability: high
credibility: confirmed
sensitivity: public
pages_updated:
  - operation-us-v-igor-turashev
  - operation-us-v-bugat-botnet-administrator
  - operation-russian-national-charged-with-decade-long-series-of-hacking-and-bank-fraud-offenses-resulting-in-tens-of-milli
  - operation-russian-national-charged-with-decade-long-series-of-hacking-and-bank-fraud-offenses-resulting-in-tens-of-milli-2
key_findings:
  - "Tri-lateral sanctions action: "U.S. Treasury OFAC + UK + Australia against Evil Corp.\""
  - "Evil Corp operated Dridex/Bugat malware causing $100M+ losses in 40+ countries."
collection_url: https://home.treasury.gov/news/press-releases/jy2623
created: 2026-04-25
raw_path: raw/press-releases/2019-12-05_treasury_sanctions-russia-based-evil-corp-tri-lateral-uk-australia.md
text_status: parsed
content_hash: sha256:6bfb90777c8983845856643221b7be4aa54de6a13eaa3de12018f936bc5b624f
word_count: 244
extraction_date: 2026-04-25
last_fetcher: urllib
---
## Source Summary

U.S. Treasury (OFAC) press release announcing the December 2019 tri-lateral (U.S./UK/AU) sanctions action against Russia-based Evil Corp members, designating Yakubets, Turashev, and the broader cybercriminal organization.

## Relevance to IC

This source provides parallel U.S. agency confirmation (OFAC alongside DOJ) and is the primary source establishing the international cooperation envelope (U.S./UK/AU joint financial sanctions) for the Evil Corp / Dridex prosecution suite.
