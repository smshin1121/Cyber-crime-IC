---
type: source
title: "Greek-Led Crime Ring Busted for Stealing Millions From US Banks"
collection_url: https://greekreporter.com/2021/02/04/greek-led-crime-ring-busted-for-stealing-millions-from-us-banks/
collection_domain: greekreporter.com
source_type: news
publisher: "Greek Reporter"
author: ""
publish_date: 2021-02-04
ingest_date: 2026-04-18
language: en
reliability: medium
credibility: corroborated
sensitivity: public
source_tier: 3
pages_updated:
  - operation-105-arrested-for-stealing-over-eur-12-million-from-us-based-banks
  - 105-arrested-for-stealing-over-eur-12-million-from-us-based-banks
key_findings:
  - "Greek Reporter highlighted that Greek nationals were identified as central actors in the network."
  - "The article corroborates arrest geography and the EUR 12 million loss figure while adding public reporting on the Greek leadership angle."
created: 2026-04-18
updated: 2026-04-18
---
## Source Summary

This report is useful secondary corroboration for the Greek-national leadership narrative in Operation Secreto.

## Relevance to IC

For the linked pages, it adds a public nationality angle that complements the official releases.
