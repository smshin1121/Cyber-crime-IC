---
type: source
title: "International gun-smuggling operation puts Metro Atlanta men in prison"
source_type: news
publisher: "Atlanta Journal-Constitution"
author: Staff
publish_date: 2018-04-17
ingest_date: 2026-04-25
language: en
reliability: medium
credibility: probably-true
sensitivity: public
pages_updated:
  - operation-us-v-sherman-jackson
key_findings:
  - "Major regional newspaper coverage of the Atlanta-based 'CherryFlavor' darknet vendor sentencing."
  - "Documents firearms exports to more than 10 countries via Black Market Reloaded."
collection_url: https://www.ajc.com/news/local/metro-atlantans-sentenced-for-running-international-online-gun-ring/JLvJonuVDbJLTjxbfKLrXO/
created: 2026-04-25
raw_path: raw/news/2018-04-17_ajc_international-gun-smuggling-metro-atlanta-prison.md
copyright_policy: summary-only
---
## Source Summary

The AJC's local coverage of the sentencing of Sherman Jackson and co-conspirators for running an online firearms-export ring on the darknet. Confirms exports to over 10 countries.

## Relevance to IC

This source provides regional newspaper coverage that confirms the international export footprint of [[operation-us-v-sherman-jackson]] and adds local-context detail to the case.
