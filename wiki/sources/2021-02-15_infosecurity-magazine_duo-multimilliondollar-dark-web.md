---
type: source
title: "Duo Charged with Multimillion-Dollar Dark Web Drug Conspiracy"
source_type: news
publisher: "Infosecurity Magazine"
author: Staff
publish_date: 2021-02-15
ingest_date: 2026-04-25
language: en
reliability: medium
credibility: reported
sensitivity: public
pages_updated:
  - operation-two-men-charged-in-multi-million-dollar-darknet-drug-distribution-conspiracy
  - operation-us-v-kevin-ombisi
  - operation-us-v-kevin-olando-ombisi
  - operation-texas-residents-sentenced-for-their-involvement-in-a-counterfeit-prescription-drug-distribution-operation
key_findings:
  - "Identifies the darknet marketplace as Empire Market"
  - "Quantifies vendor reputation: 2,000+ five-star reviews, 3,657 sales by November 2019"
collection_url: https://www.infosecurity-magazine.com/news/duo-multimilliondollar-dark-web/
created: 2026-04-25
raw_path: raw/news/2021-02-15_infosecurity-magazine_duo-multimilliondollar-dark-web.md
copyright_policy: summary-only
---
## Source Summary

Cybersecurity trade press coverage of the Ombisi/Russell darknet drug case identifying Empire Market as the venue and documenting investigative techniques (Bitcoin analysis, GPS/CCTV, postal tracking, iCloud).

## Relevance to IC

Specialized trade-press perspective with darknet-marketplace specifics (Empire Market) useful for cross-referencing other Empire-related operations.
