---
type: source
title: "Dark Web Pill Factory Leader From Tuxedo Park Gets Time In Prison"
source_type: news
publisher: "Daily Voice"
author: Staff
publish_date: 2025-01-22
ingest_date: 2026-04-25
language: en
reliability: medium
credibility: confirmed
sensitivity: public
pages_updated:
  - operation-orange-county-man-sentenced-to-75-months-for-distributing-methamphetamine-and-selling-illegal-pills-on-the-dar
  - operation-us-v-kyle-weiland
key_findings:
  - "Local Hudson Valley coverage confirms Kyle Weiland's 75-month sentence for dark-web pill manufacturing operation in Tuxedo Park, NY."
  - "Notes prior 2019 federal conviction for online pill sales — current offense committed during supervised release."
collection_url: https://dailyvoice.com/ny/suffern/dark-web-pill-factory-leader-from-tuxedo-park-gets-time-in-prison/
created: 2026-04-25
raw_path: raw/news/2025-01-22_dailyvoice_dark-web-pill-factory-leader-tuxedo-park-time-prison.md
copyright_policy: summary-only
---
## Source Summary

Daily Voice provides local press coverage of the Weiland SDNY sentencing, with emphasis on local geography (Tuxedo Park, Orange County) and Weiland's recidivist pattern.

## Relevance to IC

Local press coverage corroborating [[operation-us-v-kyle-weiland]] sentencing details.
