---
type: source
title: "Russian Nationals Charged With Hacking One Cryptocurrency Exchange And Illicitly Operating Another"
source_type: press-release
publisher: "U.S. Secret Service"
author: "USSS Public Affairs"
publish_date: 2023-06-09
ingest_date: 2026-04-25
language: en
reliability: high
credibility: confirmed
sensitivity: public
pages_updated:
  - operation-russian-nationals-charged-with-hacking-one-cryptocurrency-exchange-and-illicitly-operating-another
key_findings:
  - "Parallel agency PR from US Secret Service confirming Mt. Gox hack indictment"
collection_url: https://www.secretservice.gov/newsroom/releases/2023/06/russian-nationals-charged-hacking-one-cryptocurrency-exchange-and
created: 2026-04-25
raw_path: raw/press-releases/2023-06-09_usss_russian-nationals-charged-hacking-one-cryptocurrency-exchange-and.md
text_status: parsed
storage_mode: fulltext
content_hash: sha256:10663c4b184f8d45c29b1e9ef3d97063a50d607cf91d6145bdb2ff4066a41197
word_count: 1729
extraction_date: 2026-04-29
last_fetcher: jina
---
## Source Summary

USSS press release confirming the June 9, 2023 unsealing of charges against Bilyuchenko and Verner for the Mt. Gox hack and BTC-e operations. Direct WebFetch was Akamai-blocked; URL existence verified via search results.

## Relevance to IC

This source provides parallel federal agency confirmation (USSS) for [[operation-russian-nationals-charged-with-hacking-one-cryptocurrency-exchange-and-illicitly-operating-another]].
