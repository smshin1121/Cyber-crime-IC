---
type: source
title: "Prosecutors: Jefferson Co. man had 58,000 images of child pornography"
source_type: news
publisher: "WBRC (Birmingham)"
author: "Staff"
publish_date: 2021-05-20
ingest_date: 2026-04-25
language: en
reliability: medium
credibility: reported
sensitivity: public
pages_updated:
  - operation-jefferson-county-man-sentenced-to-6-12-years-in-prison-for-possession-of-child-pornography
  - operation-us-v-james-curtis-brasher
key_findings:
  - "Identifies FBI Birmingham Child Exploitation Human Trafficking Task Force as the investigative lead"
  - "Confirms Tor/Darknet usage as access method"
collection_url: https://www.wbrc.com/2021/05/20/prosecutors-jefferson-co-man-had-images-child-pornography/
created: 2026-04-25
---
## Source Summary

WBRC Birmingham CBS affiliate covers the Brasher sentencing with FBI task-force attribution and Tor/Darknet method confirmation.

## Relevance to IC

Local press confirmation of FBI cyber task-force investigative role. Tor/Darknet access pattern is the only IC-relevant element in this otherwise domestic prosecution.
