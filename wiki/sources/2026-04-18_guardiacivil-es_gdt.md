---
type: source
title: "Ciberdelincuencia y delitos en Internet - Grupo de Delitos Telemáticos"
collection_url: https://web.guardiacivil.es/es/institucional/conocenos/especialidades/gdt/
collection_domain: guardiacivil.es
source_type: official-guidance
publisher: "Guardia Civil"
author: ""
publish_date: 2026-04-18
ingest_date: 2026-04-18
language: es
reliability: high
credibility: confirmed
sensitivity: public
source_tier: 2
pages_updated:
  - spain-guardia-civil
  - spain
key_findings:
  - "The Guardia Civil presents the GDT as its specialized unit for cybercrime and internet offences."
  - "The page describes the GDT as supporting investigation, prevention, and citizen reporting related to cyber-enabled crime."
  - "The source is a direct institutional anchor for the role and public-facing mission of the GDT."
created: 2026-04-18
duplicate_of: "[[2026-04-17_web-guardiacivil-es_ciberdelincuencia-y-delitos-en-internet-gdt]]"
duplicate_reason: same_collection_url
duplicate_key: "https://web.guardiacivil.es/es/institucional/conocenos/especialidades/gdt/"
duplicate_normalized_at: 2026-04-26
---
## Source Summary

This official Guardia Civil page provides a direct institutional description of the Grupo de Delitos Telemáticos.

## Relevance to IC

It is a primary source for [[spain-guardia-civil]] and improves the evidence base for Spain's dual-track cybercrime structure.
