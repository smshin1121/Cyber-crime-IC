---
type: source
title: "Dark Web Administrator Sentenced to 20 Years in Prison for Narcotics Trafficking and Money Laundering"
raw_path: raw/press-releases/2018-08-28_justice-gov_united-states-v-gal-vallerius.md
source_type: press-release
publisher: "US DOJ (Southern District of Florida)"
author: ""
publish_date: 2018-08-28
ingest_date: 2026-04-17
language: en
reliability: high
credibility: confirmed
sensitivity: public
pages_updated:

collection_url: https://www.justice.gov/usao-sdfl/pr/dark-web-administrator-sentenced-20-years-prison-narcotics-trafficking-and-money
created: 2026-04-17
updated: 2026-04-18
defendant_names:
  - "Dark Web Administrator"
duplicate_of: "[[2018-08-28_justice-gov_dark-web-administrator-sentenced-to-20-years-in-prison-for-narcotics-trafficking]]"
duplicate_reason: same_collection_url
duplicate_key: https://www.justice.gov/usao-sdfl/pr/dark-web-administrator-sentenced-20-years-prison-narcotics-trafficking-and-money
duplicate_normalized_at: 2026-04-26
text_status: source-digest
storage_mode: source-digest
content_hash: sha256:0896d58cbc5e4e1a7d6251970704a69a661c093e2b10763d0385a3a26dfba42f
word_count: 145
extraction_date: 2026-04-28
---
## Source Summary

Gal Vallerius (Dream Market) Enforcement Action is a defendant-specific enforcement action page derived from [[us-v-vallerius-dream-market]]. It captures the prosecutorial or seizure stage reflected in the linked case record and preserves the operation-side catalog entry for this follow-on action.

## Relevance to IC

The `raw_path` metadata links this source page to the archived corpus record `raw/press-releases/2018-08-28_justice-gov_united-states-v-gal-vallerius.md`, preserving traceable review without classifying the page as a placeholder.