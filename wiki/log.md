...
- Process notes: Single-agent regional-focus dispatch (KR/JP/VN/IT priority). Korea.kr Synap iframe variant of L11 — body retrieved via tier-1-secondary-republication (뉴데일리 verbatim 전재 + 이데일리 Phase-1 verbatim outcomes). Korean source language preserved per CLAUDE.md multi-language rule (operation page Korean Perspective section substantive). [[challenges]] category-link broken-wikilink fixes applied.
