---
type: case
title: "New York Man Sentenced to 54 Months in Prison for Selling Counterfeit Xanax on Dark Web"
case_number: "Not specified in available source metadata"
jurisdiction: "U.S. District Court, Eastern District of Missouri"
jurisdiction_country: "[[united-states]]"
case_type: prosecution
status: sentenced
crime_charged:
  - "[[online-fraud-ic]]"
  - "[[dark-web-ic]]"
defendants:
  - name: "New York Man"
    nationality: Unknown
    status: sentenced
    sentence: ""
    location_at_arrest: ""
related_operation: "[[operation-new-york-man-sentenced-to-54-months-in-prison-for-selling-counterfeit-xanax-on-dark-web]]"
ic_elements:
  mlat_requests:
    []
  extradition: ""
  evidence_from_abroad:
    []
  foreign_arrests:
    []
  asset_freezing:
    []
cooperating_agencies:
  - "[[us-doj]]"
legal_frameworks_invoked:
  - "[[informal-cooperation]]"
mechanisms_used:
  - "[[informal-cooperation]]"
key_legal_issues:
  - "[[online-fraud-ic]]"
  - "[[dark-web-ic]]"
precedent_value: "Official source-backed record; further primary filings can refine procedural detail."
source_count: 1
sources:
  - "[[2025-11-18_justice-gov_united-states-v-john-cruz]]"
created: 2026-04-18
updated: 2026-05-02
summary: "New York Man Sentenced to 54 Months in Prison for Selling Counterfeit Xanax on Dark Web is documented in the linked source material. John Cruz (Counterfeit Xanax Dark Web Vendor) Enforcement Action is a follow-on operation catalog record tied to Us V John Cruz Dark Web. Available source coverage includes US DOJ (Eastern District of Missouri)."
---
## Summary

New York Man Sentenced to 54 Months in Prison for Selling Counterfeit Xanax on Dark Web is documented in the linked source material. John Cruz (Counterfeit Xanax Dark Web Vendor) Enforcement Action is a follow-on operation catalog record tied to Us V John Cruz Dark Web. Available source coverage includes US DOJ (Eastern District of Missouri).

## Source Coverage

- US DOJ (Eastern District of Missouri), 2025-11-18: New York Man Sentenced to 54 Months in Prison for Selling Counterfeit Xanax on Dark Web.

## Operational Timeline

- 2025-11-18: public source coverage from US DOJ (Eastern District of Missouri).

## International Cooperation Details

- Named agencies and partners include Us Doj.
- Recorded cooperation mechanisms include informal cooperation.

## Legal and Procedural Posture

- Recorded crime classification: online fraud and dark web.
- Legal or procedural basis recorded in metadata: informal cooperation.
- The record is categorized as prosecution with status sentenced.
- Related legal or operational records: Operation New York Man Sentenced To 54 Months In Prison For Selling Counterfeit Xanax On Dark Web.

## Evidence and Attribution Notes

- The cited source set is treated as the controlling public record for this page; no additional filing-level evidence is asserted beyond the listed references and metadata.
- Current metadata does not identify MLAT requests, extradition, foreign-evidence transfers, foreign arrests, or asset-freezing elements.
- Recorded cooperation mechanism metadata: Informal Cooperation.
- Comparative use should preserve the single-source limitation and avoid inferring that absent cooperation fields prove no cooperation occurred.

## References

| # | Source | Publisher | Date | URL |
|---|---|---|---|---|
| 1 | New York Man Sentenced to 54 Months in Prison for Selling Counterfeit Xanax on Dark Web | US DOJ (Eastern District of Missouri) | 2025-11-18 | https://www.justice.gov/usao-edmo/pr/new-york-man-sentenced-54-months-prison-selling-counterfeit-xanax-dark-web |
