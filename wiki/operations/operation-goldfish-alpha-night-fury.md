---
type: operation
title: "Operation Goldfish Alpha / Night Fury"
title_ko: "Operation Goldfish Alpha / Night Fury (동남아 사이버위협 대응)"
aliases: ["Goldfish Alpha", "Night Fury"]
case_id: "CYB-2020-002"
period: 2
operation_type: "arrest-sweep"
status: "completed"
enforcement_type: ["arrest", "takedown"]
outcome: "success"
timeframe:
  announced: "2020"
  start: "2019"
  end: "2020"
  ongoing: false
crime_type: "[[malware-ic]]"
target_entity: "Cryptojacking and e-commerce malware networks in Southeast Asia"
lead_agency: "[[interpol]]"
coordinating_body: "[[interpol]]"
participating_countries:
  - "[[indonesia]]"
  - "[[malaysia]]"
  - "[[thailand]]"
  - "[[philippines]]"
  - "[[singapore]]"
  - "[[vietnam]]"
  - "[[myanmar]]"
participating_agencies:
  - "[[interpol]]"
  - "[[interpol-asean-desk]]"
legal_basis: []
mechanisms_used: []
results:
  arrests: 3
  indictments: 0
  servers_seized: 0
  domains_seized: 0
  cryptocurrency_seized: ""
  decryption_keys_recovered: 0
  victims_notified: 0
  other:
    - "Cryptojacking infrastructure dismantled (Goldfish Alpha)"
    - "E-commerce targeting malware operations disrupted (Night Fury)"
    - "ASEAN regional cyber threat assessment produced"
edges:
  - source_actor: "INTERPOL"
    target_actor: "ASEAN countries"
    cooperation_type: "joint_investigation"
    legal_basis: "unknown"
    direction: "undirected"
credibility_index: 2.28
source_tier: 2
missing_fields: ["legal_basis", "mechanisms_used"]
related_cases: []
related_operations: []
challenges_encountered: []
lessons_learned: []
source_count: 1
sources:
  - "[1] INTERPOL News (2020)"
created: 2026-04-08
updated: 2026-04-08
---

## Summary

INTERPOL's ASEAN Cybercrime Operations Desk coordinated two operations -- Goldfish Alpha (targeting cryptojacking malware) and Night Fury (targeting e-commerce malware) -- across Southeast Asian nations during 2019-2020. These operations addressed key cyber threats facing the ASEAN region including botnets, phishing, BEC, banking malware, ransomware, and cryptojacking. Through information sharing and joint operations among ASEAN member states, criminal networks were dismantled and arrests were made.

## Participating Parties

**Lead Agency:**
- INTERPOL ASEAN Cybercrime Operations Desk

**Participating Countries (ASEAN nations):**
Indonesia, Malaysia, Thailand, Philippines, Singapore, Vietnam, Myanmar, and other ASEAN members

**Private Sector:**
- Various cybersecurity firms providing threat intelligence

## Results

- **3+ arrests** across ASEAN countries
- Cryptojacking infrastructure dismantled
- E-commerce targeting malware operations disrupted
- Comprehensive ASEAN cyber threat assessment published

## Follow-Up Actions

> [!warning] No public court documents found
> Web search (2026-04-17) yielded no publicly accessible court filings
> for this operation. Possible reasons: non-US jurisdiction with no
> public court records system, sealed proceedings, or operation did
> not result in formal prosecution.

## References
| # | Title | Publisher | Date | URL |
|---|-------|----------|------|-----|
| [1] | INTERPOL Report Highlights Key Cyberthreats in Southeast Asia | INTERPOL | 2020 | [원본](https://www.interpol.int/News-and-Events/News/2020/INTERPOL-report-highlights-key-cyberthreats-in-Southeast-Asia) |
