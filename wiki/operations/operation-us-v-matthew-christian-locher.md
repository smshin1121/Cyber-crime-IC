---
type: operation
title: "Matthew Christian Locher Enforcement Action"
aliases:
  - "United States v. Matthew Christian Locher"
case_id: CYB-FUP-173
period: 2
operation_role: follow-on
parent_operation: ""
operation_type: prosecution
status: absorbed
enforcement_type:
  - arrest
outcome: success
timeframe:
  announced: 2022-08-29
  start: 2022
  end: 2022-08-29
  ongoing: false
crime_type: "[[online-fraud-ic]]"
crime_types:
  - "[[online-fraud-ic]]"
target_entity: "Matthew Christian Locher"
lead_agency: "[[us-doj]]"
coordinating_body: "[[us-doj]]"
participating_countries:
  - "[[united-states]]"
jurisdictions:
  - "[[united-states]]"
participating_agencies:
  - "[[us-doj]]"
organizations:
  - "[[us-doj]]"
legal_basis:
  - "[[informal-cooperation]]"
mechanisms_used:
  - "[[informal-cooperation]]"
results:
  arrests: 0
  indictments: 0
  servers_seized: 0
  domains_seized: 0
  cryptocurrency_seized: ""
  decryption_keys_recovered: 0
  victims_notified: 0
  other:
    - "Defendant-specific follow-on action derived from [[us-v-matthew-christian-locher]]"
edges:
  []
credibility_index: 4.0
source_tier: 1
missing_fields:
  []
related_cases:
  - "[[us-v-matthew-christian-locher]]"
related_operations:
  []
challenges_encountered:
  []
lessons_learned:
  []
source_count: 1
sources:
  - "[[2026-04-18_justice-gov_former-south-bay-resident-pleads-guilty-sexual-exploitation-charge-enticing-girls-send]]"
summary: "This domestic-only U.S. follow-on record has been absorbed into the canonical case page [[us-v-matthew-christian-locher]]. No visible cross-border mechanism is documented, so it is not treated as a separate international-cooperation operation."
created: 2026-04-18
updated: 2026-04-22
---
## Summary

This domestic-only U.S. follow-on record has been absorbed into the canonical case page [[us-v-matthew-christian-locher]]. No visible cross-border mechanism is documented, so it is not treated as a separate international-cooperation operation.

## Canonical Record

- [[us-v-matthew-christian-locher]]

## References

| # | Source | Publisher | Date | URL |
|---|---|---|---|---|
| [1] | Former South Bay Resident Pleads Guilty to Sexual Exploitation Charge for Enticing Girls to Send Him Images Depicting Masochism | US DOJ USAO | 2022-08-29 | https://www.justice.gov/usao-cdca/pr/former-south-bay-resident-pleads-guilty-sexual-exploitation-charge-enticing-girls-send |
