---
type: operation
title: "Logan Christopher Murfin Enforcement Action"
aliases:
  - "United States v. Logan Christopher Murfin"
case_id: CYB-FUP-367
period: 3
operation_role: follow-on
parent_operation: ""
operation_type: indictment
status: absorbed
enforcement_type:
  - indictment
outcome: success
timeframe:
  announced: 2025-12-11
  start: 2025
  end: 2025-12-11
  ongoing: true
crime_type: "[[online-fraud-ic]]"
crime_types:
  - "[[online-fraud-ic]]"
target_entity: "Logan Christopher Murfin"
lead_agency: "[[us-doj]]"
coordinating_body: "[[us-doj]]"
participating_countries:
  - "[[united-states]]"
jurisdictions:
  - "[[united-states]]"
participating_agencies:
  - "[[us-doj]]"
organizations:
  - "[[us-doj]]"
legal_basis:
  - "[[informal-cooperation]]"
mechanisms_used:
  - "[[informal-cooperation]]"
results:
  arrests: 0
  indictments: 1
  servers_seized: 0
  domains_seized: 0
  cryptocurrency_seized: ""
  decryption_keys_recovered: 0
  victims_notified: 0
  other:
    - "Defendant-specific follow-on action derived from [[us-v-logan-christopher-murfin]]"
edges:

credibility_index: 4.0
source_tier: 1
missing_fields:

related_cases:
  - "[[us-v-logan-christopher-murfin]]"
related_operations:

challenges_encountered:

lessons_learned:

source_count: 1
sources:
  - "[[2025-12-11_ktul_skiatook-man-indicted-10-charges-threats-federal-law-enforcement]]"
summary: "This domestic-only U.S. follow-on record has been absorbed into the canonical case page [[us-v-logan-christopher-murfin]]. No visible cross-border mechanism is documented, so it is not treated as a separate international-cooperation operation."
created: 2026-04-18
updated: 2026-04-26
---
## Summary

This domestic-only U.S. follow-on record has been absorbed into the canonical case page [[us-v-logan-christopher-murfin]]. No visible cross-border mechanism is documented, so it is not treated as a separate international-cooperation operation.

## Canonical Record

- [[us-v-logan-christopher-murfin]]

## References

| # | Title | Publisher | Date | URL |
|---|---|---|---|---|
| [1] | Skiatook man indicted on 10 charges related to threats on federal law enforcement | KTUL ABC8 Tulsa | 2025-12-11 | https://ktul.com/news/local/skiatook-man-indicted-on-10-charges-related-to-threats-on-federal-law-enforcement-court-documents-social-media-assault-murder-federal-agents-threats |
