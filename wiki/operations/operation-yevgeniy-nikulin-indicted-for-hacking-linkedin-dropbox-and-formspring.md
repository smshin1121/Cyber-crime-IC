---
type: operation
title: "Yevgeniy Nikulin Indicted For Hacking LinkedIn, Dropbox, And Formspring Enforcement Action"
aliases:
  - "Yevgeniy Nikulin Indicted For Hacking LinkedIn, Dropbox, And Formspring"
case_id: CYB-FUP-1223
period: 1
operation_role: follow-on
parent_operation: ""
operation_type: indictment
status: absorbed
enforcement_type:
  - indictment
outcome: success
timeframe:
  announced: 2016-10-21
  start: 2016
  end: 2016-10-21
  ongoing: true
crime_type: "[[malware-ic]]"
crime_types:
  - "[[malware-ic]]"
target_entity: "Yevgeniy Nikulin Indicted For Hacking LinkedIn, Dropbox, And Formspring"
lead_agency: "[[us-doj]]"
coordinating_body: "[[us-doj]]"
participating_countries:
  - "[[united-states]]"
jurisdictions:
  - "[[united-states]]"
participating_agencies:
  - "[[us-doj]]"
organizations:
  - "[[us-doj]]"
legal_basis:
  - "[[informal-cooperation]]"
mechanisms_used:
  - "[[informal-cooperation]]"
results:
  arrests: 0
  indictments: 1
  servers_seized: 0
  domains_seized: 0
  cryptocurrency_seized: ""
  decryption_keys_recovered: 0
  victims_notified: 0
  other:
    - "Defendant-specific follow-on action derived from [[yevgeniy-nikulin-indicted-for-hacking-linkedin-dropbox-and-formspring]]"
edges:
  []
credibility_index: 4.0
source_tier: 1
missing_fields:
  []
related_cases:
  - "[[yevgeniy-nikulin-indicted-for-hacking-linkedin-dropbox-and-formspring]]"
related_operations:
  []
challenges_encountered:
  []
lessons_learned:
  []
source_count: 1
sources:
  - "[[2026-04-18_justice-gov_yevgeniy-nikulin-indicted-hacking-linkedin-dropbox-and-formspring]]"
summary: "This domestic-only U.S. follow-on record has been absorbed into the canonical case page [[yevgeniy-nikulin-indicted-for-hacking-linkedin-dropbox-and-formspring]]. No visible cross-border mechanism is documented, so it is not treated as a separate international-cooperation operation."
created: 2026-04-18
updated: 2026-04-22
---
## Summary

This domestic-only U.S. follow-on record has been absorbed into the canonical case page [[yevgeniy-nikulin-indicted-for-hacking-linkedin-dropbox-and-formspring]]. No visible cross-border mechanism is documented, so it is not treated as a separate international-cooperation operation.

## Canonical Record

- [[yevgeniy-nikulin-indicted-for-hacking-linkedin-dropbox-and-formspring]]

## References

| # | Source | Publisher | Date | URL |
|---|---|---|---|---|
| [1] | Yevgeniy Nikulin Indicted For Hacking LinkedIn, Dropbox, And Formspring | US DOJ USAO | 2016-10-21 | https://www.justice.gov/usao-ndca/pr/yevgeniy-nikulin-indicted-hacking-linkedin-dropbox-and-formspring |
