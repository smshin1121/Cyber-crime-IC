---
type: operation
title: "Charles Thomas Abrahamson Enforcement Action"
aliases:
  - "United States v. Charles Thomas Abrahamson"
case_id: CYB-FUP-346
period: 3
operation_role: follow-on
parent_operation: ""
operation_type: indictment
status: absorbed
enforcement_type:
  - indictment
outcome: success
timeframe:
  announced: 2025-12-15
  start: 2025
  end: 2025-12-15
  ongoing: true
crime_type: "[[online-fraud-ic]]"
crime_types:
  - "[[online-fraud-ic]]"
target_entity: "Charles Thomas Abrahamson"
lead_agency: "[[us-doj]]"
coordinating_body: "[[us-doj]]"
participating_countries:
  - "[[united-states]]"
jurisdictions:
  - "[[united-states]]"
participating_agencies:
  - "[[us-doj]]"
organizations:
  - "[[us-doj]]"
legal_basis:
  - "[[informal-cooperation]]"
mechanisms_used:
  - "[[informal-cooperation]]"
results:
  arrests: 0
  indictments: 1
  servers_seized: 0
  domains_seized: 0
  cryptocurrency_seized: ""
  decryption_keys_recovered: 0
  victims_notified: 0
  other:
    - "Defendant-specific follow-on action derived from [[us-v-charles-thomas-abrahamson]]"
edges:

credibility_index: 4.0
source_tier: 1
missing_fields:

related_cases:
  - "[[us-v-charles-thomas-abrahamson]]"
related_operations:

challenges_encountered:

lessons_learned:

source_count: 1
sources:
  - "[[2025-12-15_kivitv_former-idaho-american-legion-commander-charles-abrahamson-indicted-for-embezzling-over-1-4-million]]"
summary: "This domestic-only U.S. follow-on record has been absorbed into the canonical case page [[us-v-charles-thomas-abrahamson]]. No visible cross-border mechanism is documented, so it is not treated as a separate international-cooperation operation."
created: 2026-04-18
updated: 2026-04-26
---
## Summary

This domestic-only U.S. follow-on record has been absorbed into the canonical case page [[us-v-charles-thomas-abrahamson]]. No visible cross-border mechanism is documented, so it is not treated as a separate international-cooperation operation.

## Canonical Record

- [[us-v-charles-thomas-abrahamson]]

## References

| # | Title | Publisher | Date | URL |
|---|---|---|---|---|
| [1] | Former Idaho American Legion Commander, Charles Abrahamson, indicted for embezzling over $1.4 Million | KIVI-TV (Boise) | 2025-12-15 | https://www.kivitv.com/news/local-news/in-your-neighborhood/mountain-home/former-idaho-american-legion-commander-charles-abrahamson-indicted-for-embezzling-over-1-4-million |
