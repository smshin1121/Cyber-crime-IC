---
type: operation
title: "Solomon Ekunke Okpe And Johnson Uke Obogo Enforcement Action"
aliases:
  - "United States v. Solomon Ekunke Okpe And Johnson Uke Obogo"
case_id: CYB-2023-340
period: 3
operation_role: follow-on
parent_operation: ""
operation_type: prosecution
status: absorbed
enforcement_type:
  - arrest
outcome: success
timeframe:
  announced: 2023-03-27
  start: 2023
  end: 2023-03-27
  ongoing: false
crime_type: "[[online-fraud-ic]]"
crime_types:
  - "[[online-fraud-ic]]"
target_entity: "Solomon Ekunke Okpe And Johnson Uke Obogo"
lead_agency: "[[us-doj]]"
coordinating_body: "[[us-doj]]"
participating_countries:
  - "[[united-states]]"
jurisdictions:
  - "[[united-states]]"
participating_agencies:
  - "[[us-doj]]"
organizations:
  - "[[us-doj]]"
legal_basis:
  - "[[informal-cooperation]]"
mechanisms_used:
  - "[[informal-cooperation]]"
results:
  arrests: 0
  indictments: 0
  servers_seized: 0
  domains_seized: 0
  cryptocurrency_seized: ""
  decryption_keys_recovered: 0
  victims_notified: 0
  other:
    - "Defendant-specific follow-on action derived from [[us-v-solomon-ekunke-okpe-and-johnson-uke-obogo]]"
edges:

credibility_index: 4.0
source_tier: 1
missing_fields:

related_cases:
  - "[[us-v-solomon-ekunke-okpe-and-johnson-uke-obogo]]"
related_operations:

challenges_encountered:

lessons_learned:

source_count: 1
sources:
  - "[[2023-03-27_justice-gov_united-states-v-solomon-ekunke-okpe-and-johnson-uke-obogo]]"
summary: "This domestic-only U.S. follow-on record has been absorbed into the canonical case page [[us-v-solomon-ekunke-okpe-and-johnson-uke-obogo]]. No visible cross-border mechanism is documented, so it is not treated as a separate international-cooperation operation."
created: 2026-04-17
updated: 2026-04-29
---
## Summary

This domestic-only U.S. follow-on record has been absorbed into the canonical case page [[us-v-solomon-ekunke-okpe-and-johnson-uke-obogo]]. No visible cross-border mechanism is documented, so it is not treated as a separate international-cooperation operation.

This wrapper is retained for search, backlink, and source-trace continuity only; source-backed facts and procedural analysis live in the canonical record below.

## Canonical Record

- [[us-v-solomon-ekunke-okpe-and-johnson-uke-obogo]]

## References

| # | Title | Publisher | Date | URL |
|---|---|---|---|---|
| [1] | Foreign National Sentenced for Victimizing U.S. Persons Through Cyber-Enabled Fraud Schemes | US DOJ (Office of Public Affairs) | 2023-03-27 | https://www.justice.gov/archives/opa/pr/foreign-national-sentenced-victimizing-us-persons-through-cyber-enabled-fraud-schemes |
